...
export default class CacheBase extends EventEmitter {
					...
					.map(moduleName => ()=> Promise.resolve()
						.then(()=> {
							if (this.modulesCache[moduleName]) { // Already loaded - use that
								...
								return import(`${this.modulePath}/${moduleName}.js` /* @vite-ignore */)
	...
	*/
	can(func) {
		switch (func) {
			...
			case 'lock':
				return _.isFunction(this.activeModule.lockAquire)
	...
	*/
	...
	lockAquire(key, expiry) {
		...
		return Promise.resolve(this.activeModule.lockAquire(this.settings.keyMangle(hash), expiryDate));
	}
